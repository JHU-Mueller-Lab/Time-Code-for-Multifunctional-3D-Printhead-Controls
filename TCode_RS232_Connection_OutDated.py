...
'''
Note: this code is not up to date and should only be used as a reference for how to communicate 
between the 3D printer motion control software and the python script via RS-232.

Example code for "Time Code for Multifunctional 3D Printhead Controls"
This code is formatted to work with A3200 (Aerotech) through a RS-232 network connection
...
file_folder = 'Example_30mm_3x3Checkerboard_GCODE.txt'  # this can be a single file or a folder (if performing parallel printing)
...
printerPort = openport(printer_com)
...
for file in f_list_final:
    if len(f_list_final) > 1:
        ...
        final_gcode_txt_export = path + '\\' + file_folder + '_OutputAEROTECH.txt'  # '230831_1mm_32_70_Gradient_diamond_lattice_aerotech.txt'

    else:
        gcode_txt_imported = file
        final_gcode_txt_export = gcode_txt_imported.replace('.txt','_OutputAEROTECH.txt')  # '230831_1mm_32_70_Gradient_diamond_lattice_aerotech.txt'
    ...
    # Writes uninterrupted g-code to file for use by 3D printer
    if len(f_list_final) > 1:
        final_gcode_txt_export = 'AEROTECH_' + file  # '230831_1mm_32_70_Gradient_diamond_lattice_aerotech.txt'
...
if __name__ == '__main__':
    ...
    while i < len(command_list_final):
        ...
        bytesToRead = ser.inWaiting()
        if resync_type is not False and bytesToRead > 0:
